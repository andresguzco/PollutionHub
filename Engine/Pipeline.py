...
class Pipeline:
    """
    This class represents a pipeline for training and evaluating the Extended Kalman Filter (EKF) model.

    Args:
        folderName (str): The name of the folder where the pipeline will be saved.
        modelName (str): The name of the model in the pipeline.

    Attributes:
        MSE_cv_dB_opt (None): The optimized Cross-validation Mean Squared Error (MSE) in dB.
        MSE_cv_idx_opt (None): The index of the optimized Cross-validation MSE.
        MSE_test_linear_avg (None): The average Linear Test MSE.
        MSE_test_linear_arr (None): The array of Linear Test MSE values.
        MSE_test_dB_avg (None): The average Test MSE in dB.
        MSE_test_linear_std (None): The standard deviation of the Linear Test MSE.
        test_std_dB (None): The standard deviation of the Test MSE in dB.
        Plot (None): The plot that visualizes the results.
        N_T (None): The number of training steps.
        MSE_train_dB_epoch (None): The Training MSE in dB per epoch.
        MSE_train_linear_epoch (None): The Linear Training MSE per epoch.
        MSE_cv_dB_epoch (None): The Cross-validation MSE in dB per epoch.
        MSE_cv_linear_epoch (None): The Linear Cross-validation MSE per epoch.
        N_CV (None): The number of cross-validation sequences.
        N_E (None): The number of training sequences.
        optimizer (None): The optimizer used for training.
        loss_fn (None): The loss function used for training.
        alpha (None): The composition loss factor.
        weightDecay (None): The L2 weight regularization weight decay.
        learningRate (None): The learning rate.
        N_B (None): The number of samples in each batch.
        N_steps (None): The number of training steps.
        device (None): The device (CPU or GPU) used for training.
        args (None): Arguments passed to the model.
        model (None): The model used for training and testing.
    ...
    def NNTrain(
        ...
        for ti in range(0, self.N_steps):
            ...
            for index in n_e:
                if self.args.randomLength:
                    y_training_batch[ii, :, train_lengthMask[index, :]] = \
                        train_input[index, :, train_lengthMask[index, :]]
                    train_target_batch[ii, :, train_lengthMask[index, :]] = \
                        train_target[index, :, train_lengthMask[index, :]]
                else:
    ...
    def NNTest(self, SysModel, test_input, test_target, path_results, MaskOnState=False,
        ...
        # Print MSE and std
        string = self.modelName + "-" + "MSE Test:"
        print(string, self.MSE_test_dB_avg, "[dB]")
        string = self.modelName + "-" + "STD Test:"
        print(string, self.test_std_dB, "[dB]")
        # Print Run Time
