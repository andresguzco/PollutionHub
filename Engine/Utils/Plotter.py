...
# Legend
Klegend = [
    "Unsupervised KalmanNet - Train",
    "Unsupervised KalmanNet - Validation",
...
class Plot_KF:
    ...
    The `Plot_KF` class provides methods for plotting various graphs related to the Kalman Filter algorithm
    and Neural Networks.
    ...
    **Methods:**
    ...
    - `__init__(self, folderName, modelName)`
        Constructs a new `Plot_KF` object.
    ...
    - `NNPlot_epochs(self, N_Epochs_plt, MSE_KF_dB_avg, MSE_test_dB_avg, MSE_cv_dB_epoch, MSE_train_dB_epoch)`
        Plot the MSE loss values per epoch.

        - `N_Epochs_plt` : int
            Total number of epochs to be plotted.
        - `MSE_KF_dB_avg` : float
            Average MSE loss of the Kalman Filter.
        - `MSE_test_dB_avg` : float
            Average MSE loss of the test.
        - `MSE_cv_dB_epoch` : list[float]
            MSE loss values of the cross-validation per epoch.
        - `MSE_train_dB_epoch` : list[float]
            MSE loss values of the training per epoch.

    - `KFPlot(res_grid)`
        Plot the Kalman Filter based on the given grid.

        - `res_grid` : list[list[float]]
            MSE loss values of the grid.

    - `NNPlot_test(MSE_KF_linear_arr, MSE_KF_linear_avg, MSE_KF_dB_avg, MSE_test_linear_arr, MSE_test_linear_avg,
     MSE_test_dB_avg)`
        Plot various histograms for the MSE loss values.

        - `MSE_KF_linear_arr` : torch.Tensor
            MSE loss values of the Kalman Filter in linear scale.
        - `MSE_KF_linear_avg` : float
            Average MSE loss of the Kalman Filter in linear scale.
        - `MSE_KF_dB_avg` : float
            Average MSE loss of the Kalman Filter in dB scale.
        - `MSE_test_linear_arr` : torch.Tensor
            MSE loss values of the test in linear scale.
        - `MSE_test_linear_avg` : float
            Average MSE loss of the test in linear scale.
        - `MSE_test_dB_avg` : float
            Average MSE loss of the test in dB scale.
    ...
    def NNPlot_epochs(self, N_Epochs_plt, MSE_KF_dB_avg,
        ...
        :param N_Epochs_plt: Number of epochs to plot.
        :param MSE_KF_dB_avg: The average MSE loss value for the Kalman filter.
    ...
    def KFPlot(res_grid):
        """
        :param res_grid: A 2D list containing the results of a Kalman Filter algorithm. Each row represents a
        distribution and each column represents a different noise value. The elements of
        * the list are in the following order: minus values, base values, plus values, and base NeuralEngine values.
        :return: None

        This method plots the results of a Kalman Filter algorithm using the provided res_grid. It creates a 2D
        plot with four different series, each representing a different set of values from
        * res_grid. The x-axis represents the noise values and the y-axis represents the MSE Loss Value in dB.

        The plotted series are labeled as follows:
        - "minus" series is displayed as green crosses ('xg')
        - "base" series is displayed as blue circles ('ob')
        - "plus" series is displayed as red pluses ('+r')
        - "base NeuralEngine" series is displayed as yellow circles ('oy')

        The plot is saved as an image file named 'plt_grid_dB'.

        In addition to the plot, this method also prints information about the distributions and the differences
        between the values in the res_grid. The prints are organized by distribution
        * and include the following information:
        - For each distribution:
          - The noise values for the "minus", "base", and "plus" series in dB.
        ...
        plt.plot(x_plt, res_grid[3][:], 'oy', label='base NeuralEngine')
    ...
    def NNPlot_test(
        ...
        """
        :param MSE_KF_linear_arr:
        :param MSE_KF_linear_avg:
        :param MSE_KF_dB_avg:
        :param MSE_test_linear_arr:
        :param MSE_test_linear_avg:
        :param MSE_test_dB_avg:
        :return:
...
class Plot_RTS(Plot_KF):
    """
    This class is used for plotting various data related to the RTS (Rauch-Tung-Striebel) algorithm.

    :class:`Plot_RTS` inherits from :class:`Plot_KF` and provides additional methods for plotting
    different aspects of the RTS algorithm.

    """
    def __init__(self, folderName, modelName):
        super().__init__(folderName, modelName)
        self.folderName = folderName
        self.modelName = modelName
    ...
    def NNPlot_epochs(self, N_MiniBatchTrain_plt, BatchSize, MSE_KF_dB_avg, MSE_RTS_dB_avg,
                      MSE_test_dB_avg, MSE_cv_dB_epoch, MSE_train_dB_epoch):
        """
        :param N_MiniBatchTrain_plt: The total number of mini-batches used for training.
        :param BatchSize: The size of each mini-batch.
        :param MSE_KF_dB_avg: The average MSE loss in dB for the Kalman Filter model.
        :param MSE_RTS_dB_avg: The average MSE loss in dB for the RTS model.
        :param MSE_test_dB_avg: The average MSE loss in dB for the test set.
        :param MSE_cv_dB_epoch: The MSE loss in dB for each epoch in the cross-validation set.
        :param MSE_train_dB_epoch: The MSE loss in dB for each epoch in the training set.
        :return: None

        This method plots the MSE loss values (in dB) for each epoch during training and saves the plot as an
        image file.

        Example usage:
            NNPlot_epochs(N_MiniBatchTrain_plt=1000, BatchSize=100, MSE_KF_dB_avg=0.5, MSE_RTS_dB_avg=0.4,
                          MSE_test_dB_avg=0.3, MSE_cv_dB_epoch=[0.2, 0.1, 0.05, 0.03],
                          MSE_train_dB_epoch=[0.4, 0.3, 0.2, 0.15])
        """
    ...
    def NNPlot_Hist(self, MSE_KF_linear_arr, MSE_RTS_data_linear_arr, MSE_RTSNet_linear_arr):
        """
        :param MSE_KF_linear_arr: Array of MSE values for Kalman Filter
        :param MSE_RTS_data_linear_arr: Array of MSE values for RTS Smoother
        :param MSE_RTSNet_linear_arr: Array of MSE values for RTSNet
        :return: None

        This method plots a histogram of MSE values in dB for three different models: RTSNet, Kalman Filter, and
        RTS Smoother. The input arrays should contain the MSE values for each model.
        *
        The histogram is saved as a figure in the specified folder with the name 'plt_hist_dB'. The plot includes
        labels for each model, and the x-axis represents the MSE loss value in dB, while
        * the y-axis represents the percentage.

        Example usage:
            mse_kf = [0.5, 0.3, 0.2]
            mse_rts_data = [0.4, 0.3, 0.6]
            mse_rts_net = [0.1, 0.2, 0.3]
            nn_plotter = NNPlotter()
            nn_plotter.NNPlot_Hist(mse_kf, mse_rts_data, mse_rts_net)
    ...
    @staticmethod
    def plotTraj_CA(test_target, RTS_out, rtsnet_out, dim, file_name):
...
class Plot_extended(Plot_RTS):
    def EKFPlot_Hist(self, MSE_EKF_linear_arr):
        ...
        ####################
        plt.figure(figsize=(25, 10))
        sns.distplot(10 * np.log10(MSE_EKF_linear_arr), hist=False, kde=True, kde_kws={'linewidth': 3}, color='b',
    ...
    def KF_RTS_Plot(self, r, MSE_KF_RTS_dB):
        ...
        plt.plot(x_plt, MSE_KF_RTS_dB[1, :], '--bo',
    ...
    def NNPlot_Hist(self, MSE_EKF_linear_arr, MSE_ERTS_data_linear_arr, MSE_RTSNet_linear_arr):
        ...
        ####################
        plt.figure(figsize=(25, 10))
        # sns.distplot(10 * torch.log10(MSE_RTSNet_linear_arr), hist=False, kde=True, kde_kws={'linewidth': 5},
        # color='b', label = self.modelName) sns.distplot(10 * torch.log10(MSE_EKF_linear_arr), hist=False, kde=True,
        # kde_kws={'linewidth': 3}, color= 'orange', label = 'EKF') sns.distplot(10 * torch.log10(
        # MSE_ERTS_data_linear_arr), hist=False, kde=True, kde_kws={'linewidth': 3.2,"linestyle":'--'},color= 'g',
        # label = 'RTS')
    ...
    def Partial_Plot_Lor(self, r, MSE_Partial_dB):
        ...
        main_partial.plot(x_plt, MSE_Partial_dB[0, :], '-yx', linewidth=3, markersize=12,
                          label=r'EKF:  $\rm J_{mdl}=5$')
        main_partial.plot(x_plt, MSE_Partial_dB[1, :], '--yx', linewidth=3, markersize=12,
                          label=r'EKF:  $\rm J_{mdl}=2$')
        main_partial.plot(x_plt, MSE_Partial_dB[2, :], '-bo', linewidth=3, markersize=12,
                          label=r'RTS:  $\rm J_{mdl}=5$')
        main_partial.plot(x_plt, MSE_Partial_dB[3, :], '--bo', linewidth=3, markersize=12,
                          label=r'RTS:  $ \rm J_{mdl}=2$')
        main_partial.plot(x_plt, MSE_Partial_dB[4, :], '--g^', linewidth=3, markersize=12,
        ...
        main_partial.plot(x_plt, MSE_Partial_dB[4, :], '--g^', linewidth=3, markersize=24,
    ...
    def Partial_Plot_KNetRTSNet_Compare(self, r, MSE_Partial_dB):
        ...
        NoiseFloor = -x_plt
        main_partial.plot(x_plt, NoiseFloor, '--r', linewidth=3, markersize=12, label=r'Noise Floor')
        main_partial.plot(x_plt, MSE_Partial_dB[0, :], '--bo', linewidth=3, markersize=12,
                          label=r'KNet: $\Delta{\theta}=1$')
        main_partial.plot(x_plt, MSE_Partial_dB[1, :], '--g^', linewidth=3, markersize=12,
                          label=r'RTSNet: $\Delta{\theta}=1$')
